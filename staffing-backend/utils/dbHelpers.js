...
async function findUserByResetToken(token) {
  ...
  console.log(
    `Debug: findUserByResetToken - Token: ${token}, Current Time (ms): ${currentTime}, Current Time (Date): ${new Date(
      currentTime
    ).toLocaleString()}`
  );
  ...
  if (rows[0]) {
    console.log(
      `Debug: findUserByResetToken - Found user. User ID: ${
        rows[0].id
      }, Stored resetExpires (ms): ${
        rows[0].resetExpires
      }, Stored resetExpires (Date): ${new Date(
        rows[0].resetExpires
      ).toLocaleString()}`
    );
  }
...
async function findPendingSignupByToken(token) {
  const [rows] = await pool.execute(
    "SELECT * FROM pending_signups WHERE tempToken = ?",
    [token]
  );
  return rows[0];
}
...
async function checkJobExists(jobId) {
  const [jobs] = await pool.execute("SELECT id FROM jobs WHERE id = ?", [
    jobId,
  ]);
