...
const signup = async (req, res) => {
  ...
  if (existingUser) {
    ...
    return res.json({
      message:
        "If an account with that email exists, please proceed to login or password reset.",
    });
  }
  ...
  if (pendingSignup) {
    ...
    if (pendingSignup.blockExpires && pendingSignup.blockExpires > now) {
      ...
      return res
        .status(429)
        .json({
          error: `Too many signup attempts. Please try again in ${remainingMinutes} minute(s).`,
        });
    }
    ...
    return res.status(200).json({
      message:
        "A pending signup already exists for this email. Please verify OTP or use the resend OTP endpoint.",
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to send OTP or process signup" });
  }
};
...
const resendSignupOTP = async (req, res) => {
  ...
  if (!tempToken) {
    return res
      .status(400)
      .json({ error: "Invalid or missing temporary token." });
  }
  ...
  if (pendingSignup.blockExpires && pendingSignup.blockExpires > now) {
    ...
    return res
      .status(429)
      .json({
        error: `Too many OTP send attempts. Please try again in ${remainingMinutes} minute(s).`,
      });
  }
  ...
  if (newOtpSendAttempts > MAX_OTP_SEND_ATTEMPTS) {
    ...
    return res
      .status(429)
      .json({
        error: `Too many OTP send attempts (${MAX_OTP_SEND_ATTEMPTS}). Account temporarily blocked for sending OTPs. Please try again in ${blockDurationMinutes} minute(s).`,
      });
  }
