...
export default function HrEmailOtp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Replace with your HR OTP verification API endpoint
      const res = await axios.post(`${apiUrl}/hr/verify-email-otp`, {
        email,
        otp: otpValue,
      });
      setLoading(false);
  ...
  const handleResend = async () => {
    setResendTimer(30);
    try {
      // Replace with your HR resend OTP API endpoint
      await axios.post(`${apiUrl}/hr/resend-email-otp`, { email });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#e3f0ff] via-[#f6f8fa] to-[#e3f0ff] px-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md flex flex-col items-center border border-blue-100 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col items-center">
          ...
          <button
            type="submit"
            className="w-80 flex items-center justify-center gap-2 bg-[#0F52BA] text-white py-2 rounded-lg hover:bg-[#1565C0] transition-all font-semibold disabled:opacity-60 mb-2 shadow-sm"
